...
func (db *Database) UpdateArticleSummary(id int64, summary string, engine string) error {
	_, err := db.conn.Exec(`UPDATE articles SET summary = ?, summary_engine = ? WHERE id = ?`, summary, engine, id)
	return err
}
